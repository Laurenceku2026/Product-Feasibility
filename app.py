...
PERSISTENT_API_KEY = get_secret("AI_API_KEY")
PERSISTENT_BASE_URL = get_secret("AI_BASE_URL", "https://api.deepseek.com")
PERSISTENT_MODEL_NAME = get_secret("AI_MODEL_NAME", "deepseek-chat")
...
# 注意：由于长度限制，这里省略了 TEXTS 字典，请使用您 v2 版本中的完整 TEXTS。
# 您可以直接复制原文件中的 TEXTS 内容到此处。为保持代码可运行，请务必补全。
# 以下为占位符，实际部署时请替换为您的完整 TEXTS（从 v2 文件中复制）。
TEXTS = {
    "zh": {
        "title": "📊 产品可行性 - AI分析系统",
        # ... 省略其他字段，请从 v2 文件复制完整内容
    },
    "en": {
...
# 警告：上述 TEXTS 不完整！请务必从您提供的 v2 文件中复制完整的 TEXTS 字典替换此处。
...
with st.sidebar:
    report_key_input = st.text_input(
        t.get("report_key_label", "授权码 (Report Key)"),
    ...
    # 购买引导
    st.markdown(f"## {t.get('purchase_title', '💰 购买+解锁')}")
    st.markdown("""
| 套餐 | 价格 | 次数 | 有效期 |
|------|------|------|--------|
| 单次通行 | 18元 / 3美元 | 1次 | 无限制 |
    ...
    if not product_name:
        ...
        if can_generate:
            ...
            with spinner_placeholder.container():
                st.markdown(f'<div style="text-align: center; margin-top: 10px;">{t.get("generating", "报告生成中，请稍候...")}</div>', unsafe_allow_html=True)
                with st.spinner(""):
                    try:
                        if analyst_name:
                            if analyst_title:
...
if current_report:
    ...
    if premium:
        ...
        if st.button(t.get("download_unlock_btn", "📥 下载报告+解锁"), use_container_width=True):
            purchase_dialog()
